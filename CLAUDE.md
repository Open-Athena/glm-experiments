...
- Run `make format` or `pre-commit run -a` before committing
...
pytest --version        # Verify pytest is available
...
# python glm_experiments/train.py trainer.max_epochs=20
```
...
```bash
python glm_experiments/train.py tags=["experiment", "baseline"]
```
...
8. **Test Suite**
   ```bash
   make test-full
   pytest --cov=glm_experiments --cov-report=term-missing
   ```
...
- Weights & Biases (`logger=wandb`) - Recommended
- MLFlow (`logger=mlflow`)
- Neptune (`logger=neptune`)
- Comet (`logger=comet`)
- CSV (`logger=csv`)
- Multiple (`logger=many_loggers`)
...
# Training
python glm_experiments/train.py trainer=gpu          # Train on GPU
python glm_experiments/train.py experiment=example   # Run experiment
python glm_experiments/train.py debug=fdr            # Fast debug
```
